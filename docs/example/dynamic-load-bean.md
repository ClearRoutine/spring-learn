...
详细代码可以直接参考 `spring-manual-bean-registration` 模块。
...
在进行手动注册 `Bean` 时，需要调用 `refresh()` 来初始化容器，使 `BeanFactory` 处于可用状态。这是 `Spring` 容器的必备流程，**在手动注册 Bean 之前必须先完成容器初始化**。如果通过**传入配置类的构造函数**来创建容器，`refresh()` 会在内部自动调用；而使用**无参构造函数**创建容器时，则需要手动调用 `refresh()`。